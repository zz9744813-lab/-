...
export default async function KnowledgePage({ searchParams }: { searchParams?: { q?: string; edit?: string } }) {
  ...
  return (
    <section className="space-y-6">
      <div>
        <p className="text-sm text-text-secondary">沉淀想法、资料和长期知识</p>
      ...
      <article className="rounded-lg border border-border bg-bg-surface p-6">
        ...
        <form action={createNote} className="mt-4 space-y-3">
          ...
          <textarea name="content" required rows={5} placeholder="内容" className="w-full rounded-md border border-border bg-bg-elevated px-3 py-2 text-sm" />
          <div className="flex flex-wrap gap-3">
            <input name="tags" placeholder="标签，用逗号分隔" className="min-w-0 flex-1 rounded-md border border-border bg-bg-elevated px-3 py-2 text-sm" />
            <button type="submit" className="rounded-md border border-accent bg-accent-muted px-4 py-2 text-sm">保存笔记</button>
          </div>
        </form>
      </article>
      ...
      <article className="rounded-lg border border-border bg-bg-surface p-6">
        <form className="flex gap-3">
          <input name="q" defaultValue={q} placeholder="搜索标题、内容或标签" className="min-w-0 flex-1 rounded-md border border-border bg-bg-elevated px-3 py-2 text-sm" />
          <button type="submit" className="rounded-md border border-border px-4 py-2 text-sm">搜索</button>
          {q ? <Link href="/knowledge" className="rounded-md border border-border px-4 py-2 text-sm">清除</Link> : null}
        </form>
      </article>

      {items.length === 0 ? (
        <article className="rounded-lg border border-border bg-bg-surface p-6 text-text-secondary">还没有笔记。可以先记录一个想法，或让 Hermes 从对话里整理。</article>
      ) : (
                ...
                {isEdit ? (
                  <form action={updateNote} className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Link href="/knowledge" className="rounded-md border border-border px-3 py-1.5 text-sm">取消</Link>
                    </div>
                  </form>
                ) : (
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <p className="text-xs text-text-secondary">更新于 {formatDateTime(note.updatedAt)}{note.tags ? ` · ${note.tags}` : ""}</p>
                      </div>
                      <div className="flex gap-2">
                        <Link href={`/knowledge?edit=${note.id}`} className="rounded-md border border-border px-3 py-1.5 text-sm">编辑</Link>
                        <form action={removeNote}><input type="hidden" name="id" value={note.id} /><button className="rounded-md border border-border px-3 py-1.5 text-sm text-danger">删除</button></form>
                      </div>
                    </div>
